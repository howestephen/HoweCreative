...
- [x] Replace the pillar model with a per-pixel depth map generated by `scripts/build-portrait-depth.mjs` (Depth Anything V2 small, run locally, output `public/portrait/portrait-source.png`). There are no columns. The portrait holds, the head yaws so its relief reads as parallax, then the image reduces into particles: hair and the back of the head release first while the features hold longest, and as points release the image thins to a stipple in which about a fifth of the points survive, weighted toward the brighter ones, the survivors growing to hold the coverage. The camera dollies into the cloud with near points swelling into large soft blobs and fading at the lens, then the existing flow into the field carries on unchanged.
