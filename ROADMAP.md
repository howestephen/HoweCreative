...
Keep the live editorial site and the active portfolio redesign's commercial
direction unchanged for now, apart from already-requested hero bug fixes. The
live site appears to have attracted a genuine client enquiry, so its commercial
positioning should be allowed to run while the enquiry develops and Search
Console begins collecting evidence.

Later, review a same-domain structure that combines the live site's clear AI
and systems proposition with the redesign's stronger visual portfolio. The
likely direction is a commercially focused homepage or AI consultancy page,
with the redesigned experience serving as the work portfolio. Validate the
positioning and search targets against real enquiry and search data before
changing either site.
...
- [x] Rework the first-scroll cut after physical Safari and Chrome review showed the earlier stabilisation passes still flickered and zoomed. The failure was not a poster-to-WebGL swap: point size, visibility and depth changed abruptly after a hidden release threshold, then an overlong 4.7-screen hero left several screens of dust before the work cards appeared. The corrected sequence has no first-scroll dead zone but eases its opening response, keeps size and opacity stable while the surface separates and disperses continuously, limits the camera to a shallow lateral move rather than driving it through the cloud, preserves the full particle field until Selected work begins entering, and shortens the hero to 1.7 screens on desktop and 1.75 on mobile. Small, medium and rare large particle populations are randomised independently of field depth so all three remain visible without enlarging the entire cloud. The 8443 Tailscale review URL serves the compiled preview on port 5175, so `npm run build` is required before review; port 5174 is the live source server.
...
- [x] Correct the renderer lifecycle after further first-scroll review: remove the duplicate opaque-core particle pass and its visibility thresholds; allocate a changed drawing buffer immediately before painting, never after; ignore unchanged resize notifications; wait for non-zero dimensions; keep mobile camera framing and scroll measurement stable while browser bars move. Replace the contact section's hard particle cut-off with a background-only transition, leaving page text unobscured. The renderer tests cover call ordering, first-frame readiness, unchanged/zero-size resizes, slow-frame quality changes and teardown. Deliberately restoring a post-paint clear makes the regression tests fail. These are implementation checks, not proof of physical-device rendering.
- [x] Stephen confirmed the first-scroll flicker is resolved on 22 September. Preserve the working motion and renderer at `f6067a8`. His subsequent request permits correcting the portrait's harsh neck rendering, but not changing its geometry, breakup, camera, rendering lifecycle or scroll timing.
- [x] Replace the closing broken arc with an open, tapered particle ribbon, folded through depth above the contact heading. The old arc used the survival seed as its angle, removing sectors of the shape when particles were culled. Closing placement now uses the existing independent CPU-generated seeds, avoiding the banding observed with a GPU sine hash; the target is applied only when `uEnding > 0`. A source-hash regression guard freezes the non-closing vertex shader at the accepted baseline. The 8443 preview is rebuilt for review; this records implementation, not aesthetic acceptance.
- [x] Supersede the rejected ribbon with a circulating, tilted particle volume. The stream waves through depth after the hero release, then gathers into a complete loop around the closing text. Angular placement is independent of survival; late particle size and softness follow camera distance. The loop uses world-space perspective without camera-following offsets or projection cancellation, with narrower and shorter mobile framing.
- [x] Correct portrait shadow conversion to standard sRGB and remove the added colour gain; increase point coverage to reduce gaps in the neck. The source image and accepted hero motion are unchanged. Tests compare the actual sampler against the installed Three.js conversion and retain the original hero-motion hash and frame-safe renderer checks.
- [ ] Stephen's visual review of the revised circular flow and neck tones on desktop and physical phones. Browser checks on the rebuilt 8443 route cover desktop and mobile-sized viewports, first-scroll frames, the closing composition and reversal. They do not certify physical Safari/Chrome performance or aesthetic acceptance. Opt-in sound review remains outstanding.
