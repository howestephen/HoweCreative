...
The current prototype runs at `/study`, separate from the existing homepage. The Quiver evidence-led case study is available at `/work/quiver`; it replaces the old four-cut framing with the finished launch master and inspected process artefacts from the curated portfolio pack. Browser and full-suite verification are recorded with the implementation commit. The revised portrait depth treatment remains subject to Stephen's visual review. Physical mobile-device testing and listening to the sound mix remain outstanding. The older [HTML concept board](docs/reviews/2026-09-07-design-reset/index.html) previews the previous complete image-card index. No production deployment or Figma design changed at this checkpoint. The editorial milestone below is historical.
