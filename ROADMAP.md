...
- [x] Correct the remaining first-scroll cut: hold the opening camera projection until the point cloud is visibly separating, keep the WebGL surface warm across initial wheel and touch input, use an opaque promoted canvas, deepen the early pull into and away from the lens, and replace uniform particle enlargement with a broad size distribution biased towards fine points.
